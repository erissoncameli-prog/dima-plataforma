...
function gerarLayout(tituloPagina, paginaAtiva) {
  ...
  const navHtml = navItems.map(item => {
    ...
    return `<a class="nav-item ${ativo}" href="${item.href}"><span style="font-size:14px">${item.icone}</span><span>${t('nav', item.id)}</span></a>`;
  }).join('');
  return `
  <div class="app-layout">
    ...
    <div class="main-content">
      <div class="topbar">
        ...
        <div class="topbar-breadcrumb"><span>DIMA</span><span>›</span><span>${tituloPagina}</span></div>
      </div>
...
async function initPagina(tituloPagina, paginaAtiva, callback) {
  ...
  document.getElementById('app').innerHTML = gerarLayout(tituloPagina, paginaAtiva) + `</div></div></div>`;
