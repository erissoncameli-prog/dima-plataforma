...
async function abrirModalPorEntrega(entregaId){
  ...
  if(cont&&cont.atividade_id){
    ...
    filtCont=contratoId;
    var r2=await db.from('contratos_produtos').select('*,contratos(id,numero,objeto_pt,atividade_id,fornecedores(id,nome),atividades(id,codigo,nome_pt)),contratos_produtos_entregas(nota_tecnica_url,nota_tecnica_nome,documentos:entrega_documentos(arquivo_url,arquivo_nome,tipo_documento))').eq('contrato_id',contratoId).not('situacao','in','("pago","cancelado")').order('numero_produto',{ascending:true});
...
async function setFiltroStatus(s){
  ...
  var r=await db.from('contratos_produtos')
    .select('*,contratos(id,numero,objeto_pt,atividade_id,fornecedores(id,nome),atividades(id,codigo,nome_pt)),contratos_produtos_entregas(nota_tecnica_url,nota_tecnica_nome,documentos:entrega_documentos(arquivo_url,arquivo_nome,tipo_documento))')
...
function renderCard(p){
  ...
  if((p.situacao==='aprovado'||p.situacao==='pago')&&p.contratos_produtos_entregas&&p.contratos_produtos_entregas.length){
    var comUrl=p.contratos_produtos_entregas.filter(function(e){return e.nota_tecnica_url;});
    if(comUrl.length){
      var ultEnt=comUrl[comUrl.length-1];
      linkEntrega='<a href="'+ultEnt.nota_tecnica_url+'" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:4px;font-size:11px;color:#1D4ED8;text-decoration:none;font-weight:500;padding:3px 8px;background:#EFF6FF;border:1px solid #BFDBFE;border-radius:4px;margin-left:auto" onclick="event.stopPropagation()">&#x1F4C4; '+(ultEnt.nota_tecnica_nome?esc(ultEnt.nota_tecnica_nome):'Nota técnica')+'</a>';
    }
  }
