-- ════════════════════════════════════════════════════════════════════════
-- Diagnóstico Socioambiental · 10 — coordenação usa o app SÓ em modo treino
--
-- Decisão de 26/09/2026: coordenação continua sem aplicar ficha real (quem
-- valida não aplica), mas pode entrar no app de campo em MODO TREINO para
-- testar e treinar a equipe, com o módulo 'diagnostico_treino' (com prazo).
--   · fn_diag_pode_treinar: + coordenação com 'diagnostico_treino'
--     (técnico continua precisando de 'diagnostico' + 'diagnostico_treino');
--   · diag_enviar_ficha: coordenação só com ficha de treino;
--   · fotos: quem treina pode subir foto (o registro da ficha real continua
--     barrado pela RPC; foto órfã é inacessível a quem não a enviou).
-- ════════════════════════════════════════════════════════════════════════
...
returns boolean language sql stable security definer set search_path = public as $$
  select coalesce(
    ...
    or (fn_diag_perfil_ativo() = 'coordenacao' and tem_permissao('diagnostico_treino')),
  false)
$$;

create or replace function public.fn_diag_foto_pode_enviar(p_name text)
returns boolean language sql stable security definer set search_path = public as $$
  select (fn_diag_pode_aplicar() or fn_diag_pode_treinar())
...
revoke execute on function public.fn_diag_pode_treinar(), public.fn_diag_foto_pode_enviar(text),
  public.diag_enviar_ficha(jsonb, jsonb, jsonb) from public, anon;
grant execute on function public.fn_diag_pode_treinar(), public.fn_diag_foto_pode_enviar(text),
  public.diag_enviar_ficha(jsonb, jsonb, jsonb) to authenticated;
